...
**Dashboard: implemented** (`frontend/dashboard/src/lib/clientId.ts`): `getOrCreateClientId()` generates a `crypto.randomUUID()` on first call and persists it in `localStorage` (the same mechanism 3.6's cache already uses) under its own key, independent of any Read API call. Never throws — storage unavailable (private browsing, quota) falls back to one id cached in module memory for the rest of that page session (not a fresh id per call, fixed after a PR #18 CodeRabbit finding: a fresh fallback id per call let two calls in one session, e.g. `NotificationToggle.tsx` enabling then disabling, silently act on two different "clients"). This is plain frontend TypeScript with no backend/AWS dependency — unlike 4.1/4.4/4.5, nothing here is blocked on AWS console access. **Wired**: `NotificationToggle.tsx` (4.6) and `useHeartbeat.ts` (4.4) both call it. A clientId alone is only an identifier, not proof of ownership — 4.1's client-secret hardening (`clientCredential.ts`) is the paired credential 4.5/4.6's self-service routes now also require.
...
- `GET /remote-config` also requires 4.1's `X-Client-Secret` credential matching the queried clientId (fixed after a PR #18 CodeRabbit finding: a bare clientId let anyone who knew or guessed it read that client's stored config, including 4.6's push-subscription internals) — a 403 for a missing/wrong secret or an unregistered clientId, done and tested locally.
...
- A stored new-video event is delivered at most once per client/window unless an explicit repeat-reminder policy says otherwise — done: `notification_delivery_log_store.py`'s `mark_delivered` is an atomic conditional write, claimed *before* `push_sender.py` is called rather than recorded only after a successful send (fixed after a PR #18 CodeRabbit finding: the original read-then-send-then-write order let two overlapping dispatcher runs both observe "not yet delivered" and both send). A claim moves to a permanent record only via `confirm_delivered` after a confirmed send; `release_claim` undoes a claim whose send didn't actually succeed (expired subscription, transient failure) so a later run retries immediately; and an unconfirmed claim older than `_CLAIM_EXPIRY` (20 minutes, comfortably longer than one Lambda invocation) is treated as abandoned and reclaimable — a PR #18 follow-up finding on the original atomic-claim fix itself: without this, a dispatcher invocation that claimed a pair and then died (Lambda timeout, crash) before confirming or releasing left that video permanently, silently, never delivered to that client.
