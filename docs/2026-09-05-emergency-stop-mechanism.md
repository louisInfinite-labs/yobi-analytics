...
**未測試嘅一段**:Budget 監測到真實 100% 支出 → 自動 publish 去 SNS 呢一步(冇做,因為要真係使到 $5 先會自然觸發,或者要暫時將門檻調細嚟扮爆咗——今晚冇做呢個測試)。呢個係 AWS 原生成熟功能,風險主要喺 topic policy 冇畀 `budgets.amazonaws.com` 發送權限,而 Console 嘅 Add trigger 步驟已經自動處理咗呢個授權。
