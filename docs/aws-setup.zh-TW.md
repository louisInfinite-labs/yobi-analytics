...
**更新(2026-09-05):2.3(DynamoDB Storage)同 2.4(EventBridge Daily Schedule)都已經完成部署,唔再係「之後先做」嘅未來工作。** `src/dynamodb_store.py` 早已實現並且部署緊(`YOBI_STORAGE_BACKEND=dynamodb`),8 張生產表(`YobiVideoMaster`/`YobiSnapshots` 等)全部已建立,`yobi-analytics-collector` 亦已經完全靠 DynamoDB 運作,唔再用返呢節講嘅 `/tmp` 過渡方案。EventBridge 排程方面,已經有 3 條實際運行緊嘅 schedule:主收集(18:00 JST)、discovery-only(00:00 JST)、notification dispatch(每 15 分鐘)。Trending precompute(1d/7d/30d,每個 period 2 個 batch,01:00-03:00 JST)喺 Terraform 已經拆做 6 條 schedule,但全部 `state = "DISABLED"`,並冇實際運行。詳情見 [`Roadmap.md`](../Roadmap.md)。
